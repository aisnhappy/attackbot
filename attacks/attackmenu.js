...
.setColor('RANDOM')
.setTitle('❖ Saldırı Komut Grupları')
.setTimestamp()
.addField('❖  JoinSmasher saldırısı yapmak isterseniz ', prefix + 'joinsmasher')
.addField('❖  FrameBot saldırısı yapmak isterseniz ', prefix + 'framebot')
.addField('❖  FrameBot saldırısını Türkiye proxy ile yapmak isterseniz ', prefix + 'framebot-tr')
.addField('❖  Joiner saldırısı yapmak isterseniz ', prefix + 'joiner')
.addField('❖  Cipher saldırısı yapmak isterseniz ', prefix + 'cipher')
.addField('❖  WaterFallBypass saldırısı yapmak isterseniz ', prefix + 'waterfall')
.addField('❖  JoinBurner saldırısı yapmak isterseniz ', prefix + 'joinburner')
.addField('❖  BurgerSmasher saldırısı yapmak isterseniz ', prefix + 'burgersmasher')
.addField('❖  Örnek Kullanım', prefix + 'joinsmasher deneme.com:25565')
.setFooter('© aisnhappy Tüm Hakları Saklıdır.', client.user.avatarURL)
.setTimestamp()
.setThumbnail("")
...
exports.conf = {
  enabled: true,
  guildOnly: false, 
  aliases: ['attack', 'saldırı', 'attackmenu', 'yardım'],
  permLevel: 0 
};
